...
@router.post(
    "", response_model=ProjectAssessmentOut, status_code=status.HTTP_201_CREATED
)
def create_project_assessment(
    ...
    if user.role not in ("teacher", "admin"):
        raise HTTPException(
            status_code=403,
            detail="Alleen docenten en admins kunnen beoordelingen aanmaken",
        )
...
def list_project_assessments(
    ...
    stmt = select(ProjectAssessment).where(
        ...
        ProjectAssessment.is_advisory.is_(
            False
        ),  # Exclude external assessments (consistent with overview.py)
    )
    ...
    if user.role == "admin":
        ...
        if teacher_course_ids:
            ...
            return ProjectAssessmentListResponse(
        ...
        logger.info(
            f"[STUDENT PROJECT ASSESSMENTS] User ID: {user.id}, School ID: {user.school_id}, Role: {user.role}"
        )
        ...
        logger.info(
            f"[STUDENT PROJECT ASSESSMENTS] Found {len(team_ids)} teams for student: {team_ids}"
        )
        ...
        if team_ids:
            ...
            logger.info("[STUDENT PROJECT ASSESSMENTS] Filtering by project membership")
        else:
            # No teams, return empty
            logger.info(
                "[STUDENT PROJECT ASSESSMENTS] Student has no teams, returning empty list"
            )
    ...
    if user.role == "student":
        ...
        logger = logging.getLogger(__name__)
        logger.info(
            f"[STUDENT PROJECT ASSESSMENTS] Total matching assessments after all filters: {total}"
        )
        debug_info["total_after_filters"] = total

    stmt = (
        stmt.order_by(ProjectAssessment.id.desc())
        .limit(limit)
        .offset((page - 1) * limit)
    )
    ...
    if user.role == "student":
        logger.info(
            f"[STUDENT PROJECT ASSESSMENTS] Returning {len(rows)} assessments to student"
        )
    ...
    for r in rows:
        count = (
            db.query(func.count(ProjectAssessmentScore.id))
            .filter(
    ...
    for r in rows:
        count = (
            db.query(func.count(RubricCriterion.id))
            .filter(
        ...
        for r in rows:
            if r.project_id:
                ...
                if project_team_member:
                    team = (
                        db.query(ProjectTeam)
                        .filter(ProjectTeam.id == project_team_member.project_team_id)
                        .first()
                    )
    ...
    if project_ids_with_client:
        ...
        projects_with_clients = (
            ...
            .join(ClientProjectLink, ClientProjectLink.project_id == Project.id)
            .join(Client, ClientProjectLink.client_id == Client.id)
            .filter(
        ...
        project_to_client = {
            p.id: c.organization if c else None for p, c in projects_with_clients
        }
    ...
    response_data = ProjectAssessmentListResponse(
    ...
    if user.role == "student":
        ...
        response.headers["X-Debug-Returned-Count"] = str(
            debug_info.get("returned_count", 0)
        )
        return response
...
def get_project_assessment(
    assessment_id: int,
    team_number: Optional[int] = Query(
        None, description="Filter scores by team number"
    ),
    db: Session = Depends(get_db),
    user=Depends(get_current_user),
):
    ...
    if user.role == "student":
        reflection = (
            db.query(ProjectAssessmentReflection)
            .filter(
    ...
    return ProjectAssessmentDetailOut(
        ...
        reflection=(
            ProjectAssessmentReflectionOut.model_validate(reflection)
            if reflection
            else None
        ),
...
def update_project_assessment(
    ...
    if user.role not in ("teacher", "admin"):
        raise HTTPException(
            status_code=403,
            detail="Alleen docenten en admins kunnen beoordelingen bijwerken",
        )
    ...
    if payload.rubric_id is not None:
        ...
        if rubric.scope != "project":
            raise HTTPException(
                status_code=400, detail="Rubric must have scope='project'"
            )
...
def delete_project_assessment(
    ...
    if user.role not in ("teacher", "admin"):
        raise HTTPException(
            status_code=403,
            detail="Alleen docenten en admins kunnen beoordelingen verwijderen",
        )
...
def repair_project_assessment_team_links(
    ...
    for assessment in assessments:
        ...
        existing_team_ids = set(
            row.project_team_id
            for row in db.query(ProjectAssessmentTeam.project_team_id)
            .filter(
...
def get_assessment_teams_overview(
    ...
    if user.role not in ("teacher", "admin"):
        raise HTTPException(
            status_code=403,
            detail="Alleen docenten en admins kunnen team overzicht bekijken",
        )
    ...
    for assessment_team in assessment_teams:
        ...
        project_team = (
            db.query(ProjectTeam)
            .filter(ProjectTeam.id == assessment_team.project_team_id)
            .first()
        )
...
@router.get(
    "/{assessment_id}/reflections", response_model=ProjectAssessmentReflectionsOverview
)
def get_assessment_reflections(
    ...
    if user.role not in ("teacher", "admin"):
        raise HTTPException(
            status_code=403,
            detail="Alleen docenten en admins kunnen reflecties bekijken",
        )
    ...
    # Get all reflections
    reflections_data = (
        db.query(ProjectAssessmentReflection, User)
        .join(User, ProjectAssessmentReflection.user_id == User.id)
        .filter(
...
@router.post(
    "/{assessment_id}/scores/batch", response_model=List[ProjectAssessmentScoreOut]
)
def batch_create_update_scores(
    ...
    if user.role not in ("teacher", "admin"):
        raise HTTPException(
            status_code=403, detail="Alleen docenten en admins kunnen scores toevoegen"
        )
...
@router.post(
    "/{assessment_id}/reflection", response_model=ProjectAssessmentReflectionOut
)
def create_or_update_reflection(
    ...
    if not is_member:
        raise HTTPException(
            status_code=403, detail="Not authorized to reflect on this assessment"
        )
...
@router.get(
    "/{assessment_id}/scores-overview", response_model=ProjectAssessmentScoresOverview
)
def get_assessment_scores_overview(
    ...
    if user.role not in ("teacher", "admin"):
        raise HTTPException(
            status_code=403,
            detail="Alleen docenten en admins kunnen score overzicht bekijken",
        )
    ...
    for team_num in sorted(teams_dict.keys()):
        ...
        members = [
            TeamMemberInfo(id=u.id, name=u.name, email=u.email) for u in team_members
        ]
...
@router.get(
    "/{assessment_id}/students-overview",
    response_model=ProjectAssessmentStudentsOverview,
)
def get_assessment_students_overview(
    ...
    if user.role not in ("teacher", "admin"):
        raise HTTPException(
            status_code=403,
            detail="Alleen docenten en admins kunnen leerlingenoverzicht bekijken",
        )
    ...
    students_with_teams.sort(
        key=lambda x: (x[0].class_name or "", x[1] or 0, x[0].name or "")
    )
...
def get_self_assessment(
    ...
    if user.role != "student":
        raise HTTPException(
            status_code=403, detail="Only students can access self-assessments"
        )
    ...
    if not pa:
        raise HTTPException(
            status_code=404, detail="Assessment not found or not yet available"
        )
    ...
    if self_assessment:
        ...
        scores = (
            ...
            .filter(
                ProjectAssessmentSelfAssessmentScore.self_assessment_id
                == self_assessment.id,
...
def create_or_update_self_assessment(
    ...
    if user.role != "student":
        raise HTTPException(
            status_code=403, detail="Only students can submit self-assessments"
        )
    ...
    if not is_member:
        raise HTTPException(
            status_code=403, detail="Not authorized to assess this project"
        )
    ...
    valid_criterion_ids = set(db.execute(valid_criterion_ids_query).scalars().all())
...
@router.get(
    "/{assessment_id}/self/overview", response_model=ProjectAssessmentSelfOverview
)
def get_self_assessment_overview(
    ...
    for team_num in sorted(teams_dict.keys()):
        ...
        total_scores = [
            s.total_score for s in completed_students if s.total_score is not None
        ]
        avg_total_score = (
            sum(total_scores) / len(total_scores) if total_scores else None
        )
    ...
    # Apply sorting
    if sort == "team":
        team_overviews.sort(key=lambda t: t.team_number, reverse=(direction == "desc"))
    elif sort == "name":

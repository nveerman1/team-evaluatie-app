...
def check_no_null_scores(db: Session) -> bool:
    ...
    null_pa_scores = (
        ...
        .filter(ProjectAssessmentScore.score.is_(None))
        .scalar()
    )
    if null_pa_scores > 0:
        errors.append(
            f"ProjectAssessmentScore table has {null_pa_scores} null score values"
        )

    # Check ProjectAssessmentSelfAssessmentScore.score is not null
    null_sa_scores = (
        db.query(func.count(ProjectAssessmentSelfAssessmentScore.id))
        .filter(ProjectAssessmentSelfAssessmentScore.score.is_(None))
        .scalar()
    )
    if null_sa_scores > 0:
        errors.append(
            f"ProjectAssessmentSelfAssessmentScore table has {null_sa_scores} null score values"
        )
...
def check_referential_integrity(db: Session) -> bool:
    ...
    orphan_users = (
        ...
        .filter(~User.school_id.in_(select(School.id)))
        .scalar()
    )
    if orphan_users > 0:
    ...
    orphan_courses = (
        ...
        .filter(~Course.school_id.in_(select(School.id)))
        .scalar()
    )
    if orphan_courses > 0:
    ...
    orphan_projects = (
        ...
        .filter(
            Project.course_id.isnot(None), ~Project.course_id.in_(select(Course.id))
        )
        .scalar()
    )
    ...
    orphan_allocations = (
        ...
        .filter(~Allocation.evaluation_id.in_(select(Evaluation.id)))
        .scalar()
    )
    ...
    orphan_scores = (
        ...
        .filter(~Score.allocation_id.in_(select(Allocation.id)))
        .scalar()
    )
    if orphan_scores > 0:
...
def check_unique_constraints(db: Session) -> bool:
    ...
    if duplicate_schools:
        for name, count in duplicate_schools:
            errors.append(
                f"School name '{name}' appears {count} times (should be unique)"
            )
    ...
    if duplicate_users:
        for school_id, email, count in duplicate_users:
            errors.append(
                f"User email '{email}' in school {school_id} appears {count} times (should be unique)"
            )
...
def check_business_rules(db: Session) -> bool:
    ...
    unenrolled_students = (
        db.query(func.count(User.id))
        .filter(
            User.role == "student", ~User.id.in_(select(CourseEnrollment.student_id))
        )
        .scalar()
    )
    if unenrolled_students > 0:
        warnings.append(
            f"{unenrolled_students} students are not enrolled in any course"
        )

    # Check that all evaluations have at least one allocation
    evaluations_without_allocations = (
        db.query(func.count(Evaluation.id))
        .filter(~Evaluation.id.in_(select(Allocation.evaluation_id)))
        .scalar()
    )
    if evaluations_without_allocations > 0:
        warnings.append(
            f"{evaluations_without_allocations} evaluations have no allocations"
        )

    # Check that all rubrics have at least one criterion
    rubrics_without_criteria = (
        db.query(func.count(Rubric.id))
        .filter(~Rubric.id.in_(select(RubricCriterion.rubric_id)))
        .scalar()
    )

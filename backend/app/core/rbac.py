...
def get_accessible_course_ids(db: Session, user: User) -> List[int]:
    ...
    if user.role == "admin":
        courses = (
            db.query(Course.id)
            .filter(Course.school_id == user.school_id, Course.is_active == True)
            .all()
        )
    ...
    if user.role == "teacher":
        teacher_courses = (
            ...
            .filter(
                TeacherCourse.teacher_id == user.id, TeacherCourse.is_active == True
            )
            .all()
        )
    ...
    if user.role == "student":
        ...
        courses = (
            ...
            .filter(GroupMember.user_id == user.id, GroupMember.active == True)
            .distinct()
            .all()
        )

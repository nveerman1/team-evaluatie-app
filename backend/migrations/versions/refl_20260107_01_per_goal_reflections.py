...
def upgrade():
    ...
    op.drop_constraint(
        "uq_competency_reflection_once", "competency_reflections", type_="unique"
    )
    ...
    op.alter_column(
        "competency_reflections", "goal_id", existing_type=sa.Integer(), nullable=False
    )
...
def downgrade():
    ...
    op.alter_column(
        "competency_reflections", "goal_id", existing_type=sa.Integer(), nullable=True
    )

    # Drop the per-goal unique constraint
    op.drop_constraint(
        "uq_competency_reflection_per_goal", "competency_reflections", type_="unique"
    )

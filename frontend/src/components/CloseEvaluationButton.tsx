...
type CloseEvaluationButtonProps = {
  evaluationId: number;
  currentStatus: string;
  onClose?: () => void;
};

export default function CloseEvaluationButton({
  evaluationId,
  currentStatus,
  onClose,
}: CloseEvaluationButtonProps) {
  ...
  const handleClose = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(data.detail || "Failed to close evaluation");
      }
  ...
  if (isClosed) {
    return (
      <div className="inline-flex items-center gap-2 px-4 py-2 bg-gray-100 text-gray-700 rounded-lg border border-gray-300">
        ...
        <span className="font-medium">Evaluation Closed</span>
      </div>
    );
  }

...
def get_overview_all_items(
    ...
    if not type_filter or type_filter == "project":
        ...
        project_query = (
            ...
            .join(Group, ProjectAssessment.group_id == Group.id)
            .join(
                User,
                User.id == ProjectAssessment.teacher_id,  # Teacher as creator
            )
        ...
        for assessment, teacher, course, group in project_query.all():
            ...
            members = (
                db.query(User).join(Group.members).filter(Group.id == group.id).all()
            )
...
def get_overview_matrix(
    ...
    for assessment, course, group in project_query.all():
        ...
        members = (
            db.query(User)
            .join(Group.members)
            .filter(Group.id == group.id, ~User.archived, User.role == "student")
            .all()
        )
        ...
        for member in members:
            ...
            score = (
                _calculate_project_score(
                    db, assessment.id, assessment.rubric_id, team_num
                )
                if team_num
                else None
            )
    ...
    for evaluation, course in eval_query.all():
        ...
        if student_name:
            students_in_eval = [
                s for s in students_in_eval if student_name.lower() in s.name.lower()
            ]
    ...
    for assessment, group, course, project, client in results:
        ...
        projects.append(
            ProjectOverviewItem(
                ...
                year=year,
                num_teams=num_teams,
                average_score_overall=round(average_score_overall, 1)
                if average_score_overall
                else None,
...
def get_academic_years(
    ...
    academic_years = (
        ...
        .filter(AcademicYear.school_id == school_id, AcademicYear.is_archived == False)
...
def get_courses_for_overview(
    ...
    courses = (
        db.query(Course)
        .filter(Course.school_id == school_id, Course.is_active == True)
...
def get_peer_evaluation_dashboard(
    ...
    students_query = db.query(User).filter(
        User.school_id == school_id, User.role == "student", User.archived == False
    )
                ...
                if eval_scores:  # Only add if there are scores
                    student_evaluations.append(
                        PeerEvaluationDetail(
                            ...
                            label=project_name,
                            scores=eval_scores,
                            teacher_scores=eval_teacher_scores
                            if eval_teacher_scores
                            else None,
                        )
                    )
...
def get_teacher_feedback(
    ...
    - project_id: Filter by specific project
    """
    from app.api.v1.schemas.overview import TeacherFeedbackResponse

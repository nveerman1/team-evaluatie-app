...
def _calculate_project_score(
    ...
    criteria = (
        db.query(RubricCriterion).filter(RubricCriterion.rubric_id == rubric_id).all()
    )
...
def _get_grade_value(
    grade: Grade, field_name: str, meta_key: str = None
) -> Optional[float]:
    """
...
def _calculate_peer_score(
    db: Session, evaluation_id: int, user_id: int
) -> Optional[float]:
    """
    ...
    grade = (
        db.query(Grade)
        .filter(Grade.evaluation_id == evaluation_id, Grade.user_id == user_id)
        .first()
    )
...
def _calculate_competency_score(
    db: Session, window_id: int, user_id: int
) -> Optional[float]:
    """
    ...
    """
    scores = (
        db.query(func.avg(CompetencySelfScore.score))
        .filter(
...
def get_overview_all_items(
    ...
    if not type_filter or type_filter == "project":
        ...
        project_query = (
            ...
            .join(ProjectTeam, ProjectTeam.id == ProjectAssessmentTeam.project_team_id)
            .join(User, User.id == ProjectAssessment.teacher_id)  # Teacher as creator
            .join(Project, ProjectTeam.project_id == Project.id)
            .outerjoin(Course, Project.course_id == Course.id)
            .filter(ProjectAssessment.school_id == school_id)
        )
        ...
        if teacher_id:
            project_query = project_query.filter(
                ProjectAssessment.teacher_id == teacher_id
            )
        if status:
            project_query = project_query.filter(ProjectAssessment.status == status)
        if date_from_dt:
            project_query = project_query.filter(
                ProjectAssessment.published_at >= date_from_dt
            )
        if date_to_dt:
            project_query = project_query.filter(
                ProjectAssessment.published_at <= date_to_dt
            )
        if team_number:
    ...
    if not type_filter or type_filter == "competency":
        ...
        window_query = (
            ...
            .outerjoin(Course, CompetencyWindow.course_id == Course.id)
        ...
        if date_from_dt:
            window_query = window_query.filter(
                CompetencyWindow.end_date >= date_from_dt
            )
        if date_to_dt:
            window_query = window_query.filter(
                CompetencyWindow.start_date <= date_to_dt
            )
        if search:
            search_pattern = f"%{search}%"
            window_query = window_query.filter(
                CompetencyWindow.title.ilike(search_pattern)
            )
        ...
        for window, course in window_query.all():
            ...
            students_with_scores = (
                db.query(User)
                .join(CompetencySelfScore, CompetencySelfScore.user_id == User.id)
            ...
            # Filter by student_id if specified
            if student_id:
                students_with_scores = students_with_scores.filter(
                    User.id == student_id
                )
    ...
    if sort_by == "date":
        items.sort(
            key=lambda x: x["date"] or datetime.min, reverse=(sort_order == "desc")
        )
...
def get_overview_matrix(
    ...
    logger.info(
        f"get_overview_matrix called: course_id={course_id}, class_name={class_name}, "
        f"student_name={student_name}, date_from={date_from}, date_to={date_to}"
    )
    ...
    if course_id is not None:
        ...
        if not course:
            logger.warning(f"Course {course_id} not found for school {school_id}")
    ...
    try:
        ...
        project_query = (
            ...
            .filter(
                ...
                ProjectAssessment.is_advisory.is_(
                    False
                ),  # Exclude external assessments
            )
        )
        ...
        for evaluation, course in eval_query.all():
            ...
            students_in_eval = (
                db.query(User)
                .join(Allocation, Allocation.reviewee_id == User.id)
                .filter(
            ...
            if class_name:
                students_in_eval = [
                    s for s in students_in_eval if s.class_name == class_name
                ]
            ...
            if allowed_student_ids is not None:
                students_in_eval = [
                    s for s in students_in_eval if s.id in allowed_student_ids
                ]
        ...
        for window, course in window_query.all():
            ...
            if class_name:
                students_with_scores = [
                    s for s in students_with_scores if s.class_name == class_name
                ]
            ...
            if allowed_student_ids is not None:
                students_with_scores = [
                    s for s in students_with_scores if s.id in allowed_student_ids
                ]
        ...
        evaluations.sort(
            key=lambda x: (x["date"] is None, x["date"] if x["date"] else datetime.max)
        )
        ...
        for student_id, data in student_data.items():
            ...
            # Calculate average
            average = (
                round(sum(scores_for_avg) / len(scores_for_avg), 2)
                if scores_for_avg
                else None
            )
        ...
        for col in columns:
            ...
            column_averages[col.key] = (
                round(sum(scores) / len(scores), 2) if scores else None
            )

        # ==================== RETURN MATRIX ====================
        logger.info(
            f"get_overview_matrix completed: {len(columns)} columns, {len(rows)} rows, "
        ...
        raise HTTPException(
            status_code=500, detail="Internal server error while processing matrix data"
        )
    except Exception as e:
        logger.error(
            f"Unexpected error in get_overview_matrix: {str(e)}", exc_info=True
        )
...
def get_project_overview(
    ...
    query = (
        ...
        .outerjoin(Project, ProjectAssessment.project_id == Project.id)
        .outerjoin(Course, Project.course_id == Course.id)
        .outerjoin(ClientProjectLink, ClientProjectLink.project_id == Project.id)
        .outerjoin(Client, ClientProjectLink.client_id == Client.id)
        .filter(
            ProjectAssessment.school_id == school_id,
            ProjectAssessment.is_advisory.is_(False),  # Exclude external assessments
            ProjectAssessment.status.in_(
                ["published", "closed"]
            ),  # Only show published or closed assessments
        )
    )
    ...
    if search_query:
        ...
        query = query.filter(ProjectAssessment.title.ilike(search_pattern))
    ...
    for assessment, course, project, client in results:
        ...
        num_teams = (
            db.query(func.count(func.distinct(ProjectAssessmentScore.team_number)))
            .filter(
        ...
        if not rubric:
            ...
            if not all_scores:
                ...
                for team_number, scores in team_scores.items():
                    ...
                    for criterion in criteria:
                        if criterion.id in score_map:
                            total_weighted_score += (
                                score_map[criterion.id] * criterion.weight
                            )
                            total_weight += criterion.weight

                    if total_weight > 0:
                        avg_score = total_weighted_score / total_weight
                        team_grade = _score_to_grade(
                            avg_score, rubric.scale_min, rubric.scale_max
                        )
                    ...
                    for cat, total_score in category_scores.items():
                        if category_weights[cat] > 0:
                            ...
                            cat_grade = _score_to_grade(
                                avg, rubric.scale_min, rubric.scale_max
                            )
                ...
                for cat, grades in team_category_grades.items():
                    if grades:
                        average_scores_by_category[cat] = round(
                            sum(grades) / len(grades), 1
                        )
        ...
        projects.append(
            ProjectOverviewItem(
                ...
                average_score_overall=(
                    round(average_score_overall, 1) if average_score_overall else None
                ),
    ...
    return ProjectOverviewListResponse(projects=projects, total=len(projects))
...
def get_project_teams(
    ...
    assessment = (
        db.query(ProjectAssessment)
        .filter(
            ProjectAssessment.id == project_id, ProjectAssessment.school_id == school_id
        )
        .first()
    )
    ...
    if not rubric:
        return ProjectTeamsResponse(
            project_id=project_id, project_name=assessment.title, teams=[]
        )
    ...
    # Get criteria with categories
    criteria = (
        db.query(RubricCriterion).filter(RubricCriterion.rubric_id == rubric.id).all()
    )
    ...
    project_teams = (
        ...
        .filter(ProjectTeam.project_id == assessment.project_id)
        .all()
        ...
        for cat, total_score in category_scores_dict.items():
            if category_weights_dict[cat] > 0:
                ...
                category_scores[cat] = round(
                    _score_to_grade(avg, rubric.scale_min, rubric.scale_max), 1
                )
    ...
    return ProjectTeamsResponse(
        project_id=project_id, project_name=assessment.title, teams=teams
    )
...
def get_academic_years(
    ...
    academic_years = (
        ...
        .filter(
            AcademicYear.school_id == school_id, AcademicYear.is_archived.is_(False)
        )
...
def get_peer_evaluation_dashboard(
    ...
    if not evaluation_ids:
        ...
        return PeerOverviewDashboardResponse(
            trendData=[], heatmapData=[], kpiData=KpiData()
        )
    ...
    students_query = db.query(User).filter(
        User.school_id == school_id, User.role == "student", User.archived.is_(False)
    )
    ...
    for student in students:
        ...
        if evaluations:
            for evaluation in evaluations:
                ...
                eval_student_scores = evaluation_scores_cache.get(
                    evaluation.id, {}
                ).get(student.id, {})
                eval_scores = {}
                ...
                if evaluation.settings:
                    for cat_name in eval_student_scores.keys():
                        ...
                        if teacher_key in evaluation.settings:
                            try:
                                eval_teacher_scores[cat_abbrev] = int(
                                    evaluation.settings[teacher_key]
                                )
                            except (ValueError, TypeError):
                                pass
                        else:
                ...
                if eval_scores:  # Only add if there are scores
                    student_evaluations.append(
                        PeerEvaluationDetail(
                            ...
                            scores=eval_scores,
                            teacher_scores=(
                                eval_teacher_scores if eval_teacher_scores else None
                            ),
                        )
                    )
        ...
        if student_scores:
            ...
            overall_avg = sum(s.current for s in student_scores.values()) / len(
                student_scores
            )
    ...
    if evaluations:
        ...
        for month_key in sorted(
            ...
            for cat_name, cat_scores in scores.items():
                ...
                if cat_scores:
                    normalized_scores[normalized_key] = sum(cat_scores) / len(
                        cat_scores
                    )
                else:
    ...
    top_performers = sorted(kpi_students, key=lambda x: x[1]["score"], reverse=True)[:3]

    # Grootste dalers - students with lowest scores
    bottom_performers = sorted(kpi_students, key=lambda x: x[1]["score"])[:3]

    # Structureel laag - students consistently below 3.0
    structurally_low = [
        (student_id, data) for student_id, data in kpi_students if data["score"] < 3.0
    ][:5]

    # Inconsistenties - students with large self vs peer differences
    inconsistencies = sorted(
        kpi_students, key=lambda x: x[1]["self_vs_peer_diff"], reverse=True
    )[:5]
    ...
    kpi_data = KpiData(
        ...
        inconsistenties=[
            KpiStudent(
                student_id=sid,
                student_name=data["name"],
                value=data["self_vs_peer_diff"],
            )
            for sid, data in inconsistencies
            if data["self_vs_peer_diff"] > 0
        ],
    )
    ...
    for student_row in heatmap_data[:3]:  # Log first 3 students
        print(
            f"[OVERVIEW DEBUG] Student: {student_row.student_name}, scores keys: {list(student_row.scores.keys())}"
        )
        for cat_name, score_obj in student_row.scores.items():
            print(
                f"[OVERVIEW DEBUG]   {cat_name}: current={score_obj.current}, teacher_score={score_obj.teacher_score}"
            )

    return PeerOverviewDashboardResponse(
        trendData=trend_data, heatmapData=heatmap_data, kpiData=kpi_data
    )
...
def get_peer_evaluation_feedback(
    ...
    # Get evaluations matching filters
    eval_query = db.query(Evaluation).filter(
        Evaluation.school_id == school_id, Evaluation.evaluation_type == "peer"
    )
    ...
    if not evaluation_ids:
        return FeedbackCollectionResponse(feedbackItems=[], totalCount=0)
    ...
    scores_with_comments = (
        db.query(Score, Allocation, User, Evaluation, Project)
        .join(Allocation, Allocation.id == Score.allocation_id)
        .join(User, User.id == Allocation.reviewee_id)
        .join(Evaluation, Evaluation.id == Allocation.evaluation_id)
        .outerjoin(Project, Project.id == Evaluation.project_id)
        .filter(
    ...
    for score, allocation, user, evaluation, project in scores_with_comments:
        ...
        is_risk = any(
            word in comment_lower for word in ["probleem", "onzeker", "zwak", "niet"]
        )
        ...
        feedback_type = (
            "self" if allocation.reviewer_id == allocation.reviewee_id else "peer"
        )
        ...
        if feedback_type == "peer":
            from_student = (
                db.query(User).filter(User.id == allocation.reviewer_id).first()
            )
    ...
    return FeedbackCollectionResponse(
        feedbackItems=feedback_items, totalCount=len(feedback_items)
    )


@router.get(
    "/peer-evaluations/aggregated-feedback", response_model=AggregatedFeedbackResponse
)
def get_aggregated_peer_feedback(
    ...
    if not evaluation_ids:
        return AggregatedFeedbackResponse(feedbackItems=[], totalCount=0)
    ...
    for evaluation in evaluations:
        ...
        if evaluation.project_id:
            project = (
                db.query(Project).filter(Project.id == evaluation.project_id).first()
            )
    ...
    allocations = (
        db.query(Allocation).filter(Allocation.evaluation_id.in_(evaluation_ids)).all()
    )
    ...
    scores = (
        db.query(Score)
        .filter(Score.allocation_id.in_(allocation_ids), Score.status == "submitted")
        .all()
    )
    ...
    for allocation in allocations:
        ...
        score_O = (
            sum(category_scores["O"]) / len(category_scores["O"])
            if category_scores["O"]
            else None
        )
        score_M = (
            sum(category_scores["M"]) / len(category_scores["M"])
            if category_scores["M"]
            else None
        )
        score_Z = (
            sum(category_scores["Z"]) / len(category_scores["Z"])
            if category_scores["Z"]
            else None
        )
        score_A = (
            sum(category_scores["A"]) / len(category_scores["A"])
            if category_scores["A"]
            else None
        )
        ...
        combined_feedback = (
            " | ".join(all_feedback) if all_feedback else "Geen feedback"
        )
    ...
    return AggregatedFeedbackResponse(
        feedbackItems=aggregated_items, totalCount=len(aggregated_items)
    )


@router.get(
    "/peer-evaluations/teacher-feedback", response_model=TeacherFeedbackResponse
)
def get_teacher_feedback(
    ...
    if not evaluations:
        return TeacherFeedbackResponse(feedbackItems=[], totalCount=0)
    ...
    for evaluation in evaluations:
        ...
        project_name = (
            ...
            else f"Peer Evaluatie {evaluation.id}"
        )
        eval_date = evaluation.closed_at or evaluation.created_at
    ...
    return TeacherFeedbackResponse(
        feedbackItems=teacher_feedback_items, totalCount=len(teacher_feedback_items)
    )
...
def get_peer_evaluation_reflections(
    ...
    if not evaluation_ids:
        return ReflectionResponse(reflectionItems=[], totalCount=0)

    # Get reflections from the Reflection table
    reflection_query = (
        db.query(Reflection)
        .join(User, Reflection.user_id == User.id)
        .filter(
    ...
    if student_name:
        reflection_query = reflection_query.filter(User.name.ilike(f"%{student_name}%"))
    ...
    for reflection in reflections:
        ...
        evaluation = (
            db.query(Evaluation)
            .filter(Evaluation.id == reflection.evaluation_id)
            .first()
        )
        if not evaluation:
            continue
    ...
    return ReflectionResponse(
        reflectionItems=reflection_items, totalCount=len(reflection_items)
    )

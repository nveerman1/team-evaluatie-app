...
def test_course_enrollment_creation(sample_data=None):
    ...
    for student in data["students"]:
        enrollments_to_create.append(
            {"student_id": student.id, "course_id": data["course"].id, "active": True}
        )
...
def test_enrollment_reactivation():
    ...
    existing_enrollment = {"student_id": 1, "course_id": 1, "active": False}
...
def test_enrollment_query_logic():
    ...
    assert (
        len(course_1_enrollments) == 3
    )  # Students 1, 2, and 3 all have course_id=1 and active=True

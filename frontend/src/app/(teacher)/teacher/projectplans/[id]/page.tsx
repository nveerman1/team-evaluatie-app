...
export default function ProjectPlanDetailPage() {
  ...
  const [selectedTeam, setSelectedTeam] = useState<ProjectPlanTeam | null>(
    null,
  );
  ...
  > = {
    [SectionKey.CLIENT]: {
      title: "1. Opdrachtgever",
      description: "Wie is de opdrachtgever?",
    },
    [SectionKey.PROBLEM]: {
      title: "2. Probleemstelling",
      description: "Wat is het probleem?",
    },
    [SectionKey.GOAL]: {
      title: "3. Doelstelling",
      description: "Wat is het doel?",
    },
    [SectionKey.METHOD]: {
      title: "4. Methode",
      description: "Hoe gaan jullie te werk?",
    },
    [SectionKey.PLANNING]: {
      title: "5. Planning",
      description: "Wat is jullie planning?",
    },
    [SectionKey.TASKS]: {
      title: "6. Taakverdeling",
      description: "Wie doet wat?",
    },
    [SectionKey.MOTIVATION]: {
      title: "7. Motivatie",
      description: "Waarom dit project?",
    },
    [SectionKey.RISKS]: {
      title: "8. Risico's",
      description: "Welke risico's zijn er?",
    },
  };
    ...
    try {
      ...
      if (teamParam && result.teams.length > 0) {
        const team = result.teams.find(
          (t) => t.team_number === Number(teamParam),
        );
        if (team) {
  ...
  const loadOverview = useCallback(async () => {
    try {
      const result = await projectPlanService.getProjectPlanOverview(
        projectPlanId,
        {
  ...
  useEffect(() => {
    ...
    const clientSection = selectedTeam?.sections.find(
      (s) => s.key === SectionKey.CLIENT,
    );
    ...
    projectPlanService
      .suggestClient(projectPlanId, selectedTeamId)
      .then((results) => {
  ...
  const handleTeamStatusUpdate = async (status: PlanStatus) => {
    ...
    if (
      !confirm(
        `Weet je zeker dat je dit team wilt markeren als ${status === PlanStatus.GO ? "GO" : "NO-GO"}?`,
      )
    ) {
      return;
    }
    setSavingTeam(true);
    try {
      await projectPlanService.updateTeamStatus(projectPlanId, selectedTeamId, {
        status,
      });
      await loadDetail();
      alert(
        `Team status bijgewerkt naar ${status === PlanStatus.GO ? "GO" : "NO-GO"}`,
      );
    } catch (e: any) {
      alert(e?.response?.data?.detail || e?.message || "Opslaan mislukt");
    } finally {
  ...
  const getSectionStatusBadge = (status: SectionStatus) => {
    ...
    const badge = badges[status];
    return (
      <span className={badge.className + " text-xs font-medium"}>
        {badge.label}
      </span>
    );
  };
          ...
          <div className="flex items-start justify-between">
            <div>
              ...
              <div className="flex flex-wrap gap-2 mt-2 text-sm text-slate-600">
                ...
                <span>
                  • {detail.team_count}{" "}
                  {detail.team_count === 1 ? "team" : "teams"}
                </span>
              </div>
            </div>
          </div>
                          ...
                          <div className="flex items-center gap-2">
                            <div className="w-20 h-2 bg-gray-200 rounded-full overflow-hidden">
                              <div
                                ...
                                style={{
                                  width: `${(team.sections_filled / team.sections_total) * 100}%`,
                                }}
                              />
                            </div>
                  ...
                  <div className="flex items-start justify-between">
                    <div>
                      <h3 className="text-lg font-semibold">
                        {selectedTeam.title ||
                          `Team ${selectedTeam.team_number || selectedTeam.project_team_id}`}
                      </h3>
                  ...
                  {selectedTeam.global_teacher_note && (
                    <div className="mt-3 p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
                      <p className="text-sm text-gray-700">
                        <strong>Globale feedback:</strong>{" "}
                        {selectedTeam.global_teacher_note}
                      </p>
                    </div>
                  )}
                              ...
                              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                <div className="space-y-3">
                                  ...
                                  <div className="p-3 bg-gray-50 rounded-lg border border-gray-200 min-h-[8rem] text-sm">
                                    {section.key === SectionKey.CLIENT &&
                                    section.client ? (
                                      <div className="space-y-1">
                                        {section.client.organisation && (
                                          <p>
                                        ...
                                        {section.client_id ? (
                                          <div className="flex items-center gap-2 rounded-lg bg-green-50 border border-green-200 px-3 py-2 text-sm text-green-800">
                                            ...
                                            <span>
                                              ...
                                              >
                                                {section.linked_organization ??
                                                  `opdrachtgever #${section.client_id}`}
                                              </a>
                                            </span>
                                          </div>
                                        ) : (

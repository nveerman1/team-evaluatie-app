...
def queue_summary_generation(
    ...
    if existing_job:
        ...
        return JobStatusResponse(
            ...
            started_at=(
                existing_job.started_at.isoformat() if existing_job.started_at else None
            ),
            completed_at=(
                existing_job.completed_at.isoformat()
                if existing_job.completed_at
                else None
            ),
            cancelled_at=(
                existing_job.cancelled_at.isoformat()
...
def create_scheduled_job(
    ...
    try:
        ...
        return ScheduledJobResponse(
            ...
            last_run_at=(
                scheduled_job.last_run_at.isoformat()
                if scheduled_job.last_run_at
                else None
            ),
            next_run_at=(
                scheduled_job.next_run_at.isoformat()

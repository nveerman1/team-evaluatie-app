...
function LoginForm() {
  ...
  const handleAzureLogin = () => {
    ...
    // searchParams.get() returns the URL-decoded value automatically
    ...
    <main className="min-h-screen flex flex-col justify-center p-6 max-w-xl mx-auto space-y-4">
      ...
      {devLoginEnabled && (
        <div className="p-4 border rounded-xl space-y-3 bg-yellow-50 border-yellow-300">
          ...
          <div className="bg-yellow-100 border border-yellow-400 text-yellow-800 px-3 py-2 rounded text-xs">
            <strong>Development Mode:</strong> Dit is alleen beschikbaar in
            lokale ontwikkeling. Voer een email in en klik op "Dev Login".
          </div>
          ...
          <p className="text-xs text-gray-600">
            💡 Tip: Gebruik admin@school.nl, docent@school.nl, of
            student1@school.nl
          </p>
        </div>
      )}
      ...
      {!devLoginEnabled && (
        <div className="p-4 border rounded-xl bg-gray-50 text-center text-sm text-gray-600">
          Voor lokale ontwikkeling: zet NEXT_PUBLIC_ENABLE_DEV_LOGIN=true in je
          .env.local
        </div>
      )}
    </main>

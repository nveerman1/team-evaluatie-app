...
export const competencyMonitorService = {
  ...
  async getOverview(
  ...
  ): Promise<CompetencyOverviewData> {
    ...
    const heatmapResponse = await api.get(
      `/competencies/windows/${latestWindow.id}/heatmap`,
    );
    ...
    if (windows.length > 1) {
      ...
      try {
        const prevHeatmapResponse = await api.get(
          `/competencies/windows/${previousWindow.id}/heatmap`,
        );
    ...
    if (previousHeatmapData) {
      previousHeatmapData.rows.forEach(
        (row: { user_id: number; scores: Record<number, number | null> }) => {
    ...
    const heatmapRows = heatmapData.rows.map(
      (row: {
        user_id: number;
        user_name: string;
    ...
    competencies.forEach(
      (comp: { id: number; category_name: string; category_id: number }) => {
        ...
        if (categoryMap.has(comp.category_id)) {
          ...
          const newAverage =
            (existing.averageScore * existing.numStudentsUp + averageScore) /
            count;
          if (!isNaN(newAverage)) {
    ...
    const allScores = heatmapRows.flatMap((row: HeatmapStudentRow) =>
      Object.values(row.scores).filter(
    ...
    const scans = [];
    const scanLimit =
      filters?.scanRange === "last_5"
        ? 5
        : filters?.scanRange === "all"
          ? windows.length
          : 3;
    const windowsToFetch = windows.slice(
      0,
      Math.min(scanLimit, windows.length),
    );

    for (const window of windowsToFetch) {
      try {
        const windowHeatmapResponse = await api.get(
          `/competencies/windows/${window.id}/heatmap`,
        );
        const windowHeatmapData = windowHeatmapResponse.data;

        // Calculate overall average for this scan
        const windowScores = windowHeatmapData.rows.flatMap(
          (row: { scores: Record<number, number | null> }) =>
            Object.values(row.scores).filter(
        ...
        const windowAverage =
          windowScores.length > 0
            ? windowScores.reduce((sum: number, s: number) => sum + s, 0) /
              windowScores.length
            : 0;
        ...
        windowCompetencies.forEach(
          (comp: {
            ...
            if (comp.category_id) {
              ...
              windowCatNames.set(
                comp.category_id,
                comp.category_name || `Category ${comp.category_id}`,
              );
            }
          },
        );
        ...
        windowCompetencies.forEach(
          (comp: { id: number; category_id: number }) => {
            if (!comp.category_id) return;
        ...
        const windowCategoryAverages = Array.from(
          windowCategoryScores.entries(),
        ).map(([catId, compScores]) => ({
          categoryId: catId,
          categoryName: windowCatNames.get(catId) || `Category ${catId}`,
          averageScore:
            compScores.length > 0
              ? compScores.reduce((sum: number, s: number) => sum + s, 0) /
                compScores.length
              : 0,
        }));

        console.log(
          `DEBUG: Scan ${window.id} (${window.title}) category averages:`,
        ...
        console.error(
          `Failed to fetch heatmap for window ${window.id}:`,
          error,
        );
      }
    }
    ...
    for (const row of heatmapRows) {
      Object.entries(row.scores).forEach(([catIdStr, score]) => {
        if (typeof score === "number" && score < 2.5) {
          const category = categorySummaries.find(
            (c) => c.id === Number(catIdStr),
          );
    ...
    if (windows.length >= 2) {
      try {
        ...
        const previousWindow = windows[1];
        const previousHeatmapResponse = await api.get(
          `/competencies/windows/${previousWindow.id}/heatmap`,
        );
        ...
        const previousScores = previousHeatmapData.rows.flatMap(
          (row: { scores: Record<number, number | null> }) =>
            Object.values(row.scores).filter(
              (s): s is number => s !== null && !isNaN(s),
            ),
        ...
        const previousAverage =
          previousScores.length > 0
            ? previousScores.reduce((sum: number, s: number) => sum + s, 0) /
        ...
        const previousStudentCategoryScores = new Map<
          number,
        ...
        for (const row of previousHeatmapData.rows) {
          ...
          if (allScoresForStudent.length > 0) {
            const avgScore =
              allScoresForStudent.reduce((sum, s) => sum + s, 0) /
              allScoresForStudent.length;
        ...
        for (const row of heatmapRows) {
          const currentScores = Object.values(row.scores).filter(
            (s): s is number => s !== null,
          );
          if (currentScores.length > 0) {
            const currentAvg =
              currentScores.reduce((sum, s) => sum + s, 0) /
            ...
            if (previousAvg !== undefined) {
              ...
              if (delta > 0.1) {
                ...
                // Threshold for decline
                studentsDeclined++;
              }
            }

            // Check for strong growth or decline in individual categories
            const previousCatScores = previousStudentCategoryScores.get(
              row.studentId,
            );
        ...
        console.error(
          "Failed to fetch previous window data for trend calculation:",
          error,
        );
      }
    }
  ...
  async getCategoryDetail(
    ...
    const category = overviewData.categorySummaries.find(
      (c) => c.id === categoryId,
    );
  ...
  async getStudents(
    filters?: CompetencyOverviewFilters,
  ): Promise<StudentCompetencySummary[]> {
    const overviewData = await this.getOverview(filters);

    // Transform heatmap rows to student summaries
    return overviewData.heatmapRows
      .map((row) => {
        const scores = Object.values(row.scores).filter(
          (s): s is number => s !== null,
        );
        const lastOverallScore =
          scores.length > 0
            ? scores.reduce((sum, s) => sum + s, 0) / scores.length
            : null;
    ...
    for (const window of windowsToFetch) {
      try {
        const goalsResponse = await api.get(
          `/competencies/windows/${window.id}/goals`,
          {
        ...
        const windowGoals: LearningGoalSummary[] = apiGoals.map(
          (g: {
            id: number;
            user_id: number;
            user_name: string;
  ...
  async getReflections(
    ...
    for (const window of windowsToFetch) {
      try {
        const reflectionsResponse = await api.get(
          `/competencies/windows/${window.id}/reflections`,
        );
        const apiReflections = reflectionsResponse.data.items || [];

        // Transform API response to our format
        const windowReflections: ReflectionSummary[] = apiReflections.map(
          (r: {
            id: number;
            user_id: number;
            user_name: string;
        ...
        console.error(
          `Failed to fetch reflections for window ${window.id}:`,
          error,
        );
      }
    }
  ...
  async getFilterOptions(): Promise<FilterOptions> {
    ...
    return {
      ...
      scans: windows.map(
        (w: { id: number; title: string; start_date: string }) => ({
          id: w.id,
          label: w.title,
          date: w.start_date,
        }),
  ...
  async getStudentDetail(studentId: number): Promise<StudentDetailData | null> {
    ...
    try {
      const studentOverviewResponse = await api.get(
        `/competencies/windows/${latestWindow.id}/student/${studentId}/overview`,
      );
      const data = studentOverviewResponse.data;

      // Transform to our format
      const currentCategoryScores = data.scores.map(
        (s: {
   ...
   */
  async getStudentHistoricalScores(
    studentId: number,
    courseId?: number,
  ): Promise<{
    studentId: number;
    ...
    try {
      ...
      const response = await api.get(
        `/competencies/student/${studentId}/historical-scores`,
        {
          params,
        },
      );

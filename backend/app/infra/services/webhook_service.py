        ...
        for attempt in range(max_retries):
            try:
                ...
                if response.status_code < 400:
                    ...
                    logger.warning(
                        f"Webhook delivery failed (attempt {attempt + 1}/{max_retries}): {last_error}"
                    )

            except requests.exceptions.Timeout:
                last_error = "Request timed out"
                logger.warning(
                    f"Webhook timeout (attempt {attempt + 1}/{max_retries}): {url}"
                )

            except requests.exceptions.RequestException as e:
                last_error = f"Request error: {str(e)}"
                logger.warning(
                    f"Webhook request error (attempt {attempt + 1}/{max_retries}): {e}"
                )

            except Exception as e:
                last_error = f"Unexpected error: {str(e)}"
                logger.error(
                    f"Unexpected webhook error (attempt {attempt + 1}/{max_retries}): {e}",
                    exc_info=True,
                )

        logger.error(
            f"Webhook delivery failed after {max_retries} attempts: {last_error}"
        )

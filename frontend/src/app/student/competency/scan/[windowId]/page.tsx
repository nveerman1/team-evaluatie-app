...
export default function SelfScanPage() {
  ...
  const [scores, setScores] = useState<
    Record<number, CompetencySelfScoreInput>
  >({});
      ...
      <div className={studentStyles.header.container}>
        <header className={studentStyles.header.wrapper}>
          <div className={studentStyles.header.titleSection}>
            <h1 className={studentStyles.header.title}>{window.title}</h1>
              ...
              >
                ...
                <div>
                  ...
                  <div className="grid grid-cols-5 gap-3">
                    {levels.map((level) => {
                      ...
                      const rubricLevel = compRubricLevels.find(
                        (rl: any) => rl.level === level,
                      );
                      const label =
                        rubricLevel?.label ||
                        comp.scale_labels[String(level)] ||
                        defaultLabels[level - 1] ||
                        `Niveau ${level}`;
                ...
                <div>
                  ...
                  <textarea
                    ...
                    onChange={(e) =>
                      handleExampleChange(comp.id, e.target.value)
                    }

...
export default function ProjectAssessmentsListInner() {
    ...
    try {
      ...
      const itemsWithTeams = await Promise.all(
        (response.items || []).map(async (item) => {
          try {
            const teamOverview = await projectAssessmentService.getTeamOverview(
              item.id,
            );
  ...
  const handleDelete = async (id: number) => {
    if (
      !confirm("Weet je zeker dat je deze projectbeoordeling wilt verwijderen?")
    )
      return;
    try {
            ...
            <section key={courseName} className="space-y-3">
              ...
              <div className="space-y-3">
                {groupedByCourse[courseName].map((item) => {
                  ...
                  const readyTeams =
                    item.teams?.filter((t) => t.status === "completed")
                      .length || 0;
                  const totalTeams = item.teams?.length || 0;
                  const progressPercentage =
                    totalTeams > 0 ? (readyTeams / totalTeams) * 100 : 0;
                    ...
                    >
                      ...
                      <div className="flex flex-1 flex-col gap-1">
                        ...
                        <div className="flex items-center gap-2 flex-wrap">
                          ...
                          {(() => {
                            const normalizedStatus =
                              normalizeProjectAssessmentStatus(item.status);
                            const pillProps =
                              getStatusPillProps(normalizedStatus);
                            return (
                        ...
                        <div className="text-xs text-slate-500">
                          Laatst bijgewerkt:{" "}
                          {displayDate
                            ? new Date(displayDate).toLocaleDateString("nl-NL")
                            : "Onbekend"}
                        </div>
                      </div>

...
function generateMockPeerCompareData(): PeerCompareData {
  ...
  const averages: OmzaScore = {
    organiseren:
      students.reduce((sum, s) => sum + (s.scores.organiseren || 0), 0) /
      students.length,
    meedoen:
      students.reduce((sum, s) => sum + (s.scores.meedoen || 0), 0) /
      students.length,
    zelfvertrouwen:
      students.reduce((sum, s) => sum + (s.scores.zelfvertrouwen || 0), 0) /
      students.length,
    autonomie:
      students.reduce((sum, s) => sum + (s.scores.autonomie || 0), 0) /
      students.length,
  };
    ...
    try {
      ...
      const averages: OmzaScore = {
        organiseren:
          filteredStudents.length > 0
            ? filteredStudents.reduce(
                (sum, s) => sum + (s.scores.organiseren || 0),
                0,
        ...
        zelfvertrouwen:
          filteredStudents.length > 0
            ? filteredStudents.reduce(
                (sum, s) => sum + (s.scores.zelfvertrouwen || 0),
                0,
              ) / filteredStudents.length
            : null,
        autonomie:
          filteredStudents.length > 0
            ? filteredStudents.reduce(
                (sum, s) => sum + (s.scores.autonomie || 0),
                0,
              ) / filteredStudents.length
            : null,
      };
      ...
      setError(
        e instanceof Error ? e.message : "Failed to load comparison data",
      );
    } finally {
      setLoading(false);
    }

...
def validate_sharepoint_url(url: str) -> Tuple[bool, str]:
    ...
    return (
        False,
        "Only SharePoint/OneDrive URLs are allowed (e.g., *.sharepoint.com, 1drv.ms)",
    )
    ...
    try:
        ...
        for family, _, _, _, sockaddr in addr_info:
            ...
            if ip_obj.is_loopback:
                return (
                    False,
                    "Webhook URL resolves to loopback address (blocked for security)",
                )
            ...
            if ip_obj.is_private:
                return (
                    False,
                    "Webhook URL resolves to private IP address (blocked for security)",
                )
            ...
            if ip_obj.is_link_local:
                return (
                    False,
                    "Webhook URL resolves to link-local address (blocked for security)",
                )
            ...
            if ip_obj.is_multicast or ip_obj.is_reserved:
                return (
                    False,
                    "Webhook URL resolves to multicast/reserved address (blocked for security)",
                )

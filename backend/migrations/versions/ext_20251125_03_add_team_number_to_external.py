...
def upgrade() -> None:
    # Add team_number column to project_team_externals
    op.add_column(
        "project_team_externals", sa.Column("team_number", sa.Integer(), nullable=True)
    )
...
def downgrade() -> None:
    # Drop indexes
    op.drop_index(
        "ix_project_team_external_group_team", table_name="project_team_externals"
    )
    op.drop_index(
        "ix_project_team_external_team_number", table_name="project_team_externals"
    )

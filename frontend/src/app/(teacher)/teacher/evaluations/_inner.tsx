...
export default function EvaluationsListInner() {
  ...
  const [coursesFromData, setCoursesFromData] = useState<
  ...
  async function deleteEvaluation(id: number) {
    ...
    if (
      !confirm(
        `Weet je zeker dat je de evaluatie "${evalTitle}" wilt verwijderen?`,
      )
    ) {
      return;
    }
            ...
            <section key={courseName} className="space-y-3">
              ...
              <div className="space-y-3">
                {groupedByCourse[courseName].map((e) => {
                  ...
                  const deadlineReflection = formatDate(
                    e?.deadlines?.reflection,
                  );

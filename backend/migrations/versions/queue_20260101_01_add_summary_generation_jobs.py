...
down_revision: Union[str, None] = "3deblok_20251221_01"
...
def upgrade() -> None:
    ...
    op.create_index(
        "ix_summary_generation_jobs_school_id", "summary_generation_jobs", ["school_id"]
    )
    op.create_index(
        "ix_summary_generation_jobs_job_id", "summary_generation_jobs", ["job_id"]
    )
    op.create_index("ix_summary_job_status", "summary_generation_jobs", ["status"])
    op.create_index(
        "ix_summary_job_eval_student",
        "summary_generation_jobs",
...
def downgrade() -> None:
    ...
    op.drop_index(
        "ix_summary_generation_jobs_job_id", table_name="summary_generation_jobs"
    )
    op.drop_index(
        "ix_summary_generation_jobs_school_id", table_name="summary_generation_jobs"
    )

...
def cleanup_stuck_jobs(dry_run: bool = False, older_than_minutes: int = 10):
    ...
    try:
        ...
        print(
            f"Found {len(stuck_jobs)} stuck job(s) older than {older_than_minutes} minutes:"
        )
        print()

        for job in stuck_jobs:
            age_minutes = (
                datetime.now(UTC) - job.created_at.replace(tzinfo=None)
            ).total_seconds() / 60
...
def main():
    ...
    count = cleanup_stuck_jobs(dry_run=args.dry_run, older_than_minutes=args.older_than)

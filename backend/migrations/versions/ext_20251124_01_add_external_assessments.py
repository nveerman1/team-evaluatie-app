...
def upgrade():
    ...
    op.create_table(
        ...
        sa.Column('status', sa.String(length=30), nullable=False, server_default='NOT_INVITED'),
        sa.Column(
            'created_at',
            sa.DateTime(),
            server_default=sa.text('now()'),
    ...
    op.create_index('ix_project_team_external_group', 'project_team_externals', ['group_id'], unique=False)
    ...
    op.create_index('ix_project_team_external_token', 'project_team_externals', ['invitation_token'], unique=True)

    # Add external evaluator fields to project_assessments table
    op.add_column('project_assessments', sa.Column('external_evaluator_id', sa.Integer(), nullable=True))
    op.add_column('project_assessments', sa.Column('role', sa.String(length=20), nullable=False, server_default='TEACHER'))
    op.add_column('project_assessments', sa.Column('is_advisory', sa.Boolean(), nullable=False, server_default='false'))
    ...
    op.add_column('rubric_criteria', sa.Column('visible_to_external', sa.Boolean(), nullable=False, server_default='true'))
...
def downgrade():
    ...
    # Remove foreign key
    op.drop_constraint('fk_project_assessments_external_evaluator', 'project_assessments', type_='foreignkey')

...
export const studentHeader = {
  ...
  userAvatar:
    "flex h-10 w-10 items-center justify-center rounded-full bg-white/15 ring-1 ring-white/20 font-semibold",
} as const;
...
export const studentNavigation = {
  ...
  // Tabs list styling
  tabsList:
    "h-11 w-full justify-start gap-1 rounded-2xl bg-white p-1 shadow-sm sm:w-auto",

  // Individual tab trigger
  tabTrigger:
    "relative rounded-xl px-4 data-[state=active]:bg-slate-800 data-[state=active]:text-white data-[state=active]:shadow-sm",
  ...
  searchInput:
    "h-11 rounded-2xl bg-white pl-9 shadow-sm ring-1 ring-slate-200 focus-visible:ring-2 focus-visible:ring-indigo-500",
} as const;
...
export const studentCards = {
  ...
  listCard: {
    container:
      "rounded-2xl border-slate-200 bg-white shadow-sm hover:shadow-md transition-shadow",
    content: "p-5",
    flexContainer:
      "flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between",
...
export const studentBadges = {
  ...
  infoPillHover:
    "rounded-full bg-indigo-50 text-indigo-700 cursor-pointer hover:bg-indigo-100",
...
export const studentUtils = {
   ...
   */
  getStatusText: (isOpen: boolean) => (isOpen ? "Open" : "Gesloten"),
...
export const studentOverview = {
  // StatPill component
  statPill:
    "flex items-center gap-2 rounded-full border bg-white/70 px-3 py-1 text-sm shadow-sm",

...
export function normalizeProjectAssessmentStatus(
  ...
  if (normalized === "published" || normalized === "gepubliceerd")
    return "published";
  ...
  switch (status) {
    case "open":
      return {
        label: "Open",
        className:
          "inline-flex items-center gap-1 rounded-full bg-blue-50 px-2 py-0.5 text-xs font-medium text-blue-700 ring-1 ring-blue-100",
      };
    case "published":
      return {
        label: "Gepubliceerd",
        className:
          "inline-flex items-center gap-1 rounded-full bg-green-50 px-2 py-0.5 text-xs font-medium text-green-700 ring-1 ring-green-100",
      };
    case "draft":
    default:
      return {
        label: "Concept",
        className:
          "inline-flex items-center gap-1 rounded-full bg-amber-50 px-2 py-0.5 text-xs font-medium text-amber-700 ring-1 ring-amber-100",
      };
  }
}
...
export function getStatusChangeMessage(
  newStatus: ProjectAssessmentStatus,
): string {
  switch (newStatus) {
    case "draft":

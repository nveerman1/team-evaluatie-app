...
class RFIDScanner:
    ...
    def __init__(self):
        ...
        if "your-app-domain.com" in API_BASE_URL:
            logger.error(
                "❌ API_BASE_URL not configured! Update the script with your backend URL."
            )
            sys.exit(1)

        if "your-api-key-here" in API_KEY:
            logger.error(
                "❌ API_KEY not configured! Update the script with your API key."
            )
            sys.exit(1)
        ...
        for attempt in range(1, RETRY_ATTEMPTS + 1):
            try:
                ...
                if response.status_code == 503:
                    logger.error(
                        "❌ Service unavailable - RFID not configured on backend"
                    )
                    return None
                ...
                logger.warning(
                    f"Connection error (attempt {attempt}/{RETRY_ATTEMPTS}): {e}"
                )

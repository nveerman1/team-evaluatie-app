...
def seed_peer_evaluations():
    ...
    try:
        ...
        for rubric in rubrics:
            criteria_count = db.query(RubricCriterion).filter(
        ...
        for eval_data in evaluations_data:
            ...
            if existing:
                ...
                print(f"  Deleting existing evaluation and related data to recreate...")
                # Delete existing data
                allocations = db.query(Allocation).filter(
                    Allocation.evaluation_id == existing.id
                ).all()
                for allocation in allocations:
                    db.query(Score).filter(Score.allocation_id == allocation.id).delete()
                db.query(Allocation).filter(Allocation.evaluation_id == existing.id).delete()
                db.query(Reflection).filter(Reflection.evaluation_id == existing.id).delete()
                db.delete(existing)
                db.commit()
            ...
            for student in students:
                ...
                reviewees = random.sample(other_students, min(num_peer_reviews, len(other_students)))
            ...
            if evaluation.status == "closed":
                for student in students:
                    ...
                    submitted_at = evaluation.closed_at - timedelta(days=days_before_close)
        ...
        for evaluation in created_evaluations:
            allocation_count = db.query(Allocation).filter(
                Allocation.evaluation_id == evaluation.id
            ).count()
            score_count = db.query(Score).join(Allocation).filter(
                Allocation.evaluation_id == evaluation.id
            ).count()

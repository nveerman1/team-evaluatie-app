...
const STATUS_OPTIONS: {
  ...
  { value: "in_progress", label: "Bezig", description: "Ik ben hiermee bezig" },
  {
    value: "submitted",
    label: "Ingeleverd",
    description: "Ik heb deze afgerond",
  },
];
...
export default function GoalPage() {
  ...
  const filteredTrainings = useMemo(() => {
    ...
    return allTrainings.filter(
      (item) =>
        item.training.competency_category_id ===
          selectedCompetency.category_id && item.training.is_active,
    );
  ...
  async function handleTrainingStatusChange(
    ...
    try {
      ...
      setAllTrainings((prev) =>
        prev.map((t) =>
          t.training.id === item.training.id
      ...
      <div className={studentStyles.header.container}>
        <header className={studentStyles.header.wrapper}>
          <div className={studentStyles.header.titleSection}>
            <h1 className={studentStyles.header.title}>Leerdoel Instellen</h1>
                ...
                >
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      ...
                      <p className={studentStyles.typography.cardTitle}>
                        {goal.goal_text}
                      </p>
                    ...
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <p className="text-xs text-slate-500 mt-1">
                          Gebruik het dropdown-menu in de kolom &apos;Mijn
                          planning&apos; om aan te geven welke trainingen je van
                          plan bent te doen.
                        </p>
                      </div>
                  ...
                  <div>
                    {trainingsLoadFailed ? (
                      <div className="p-6 text-sm text-amber-700 text-center bg-amber-50 border-t border-amber-100">
                        ⚠️ Trainingen konden niet worden geladen. Probeer de
                        pagina te vernieuwen.
                      </div>
                    ) : filteredTrainings.length === 0 ? (
                      <div className="p-6 text-sm text-slate-500 text-center">
                        Geen vaardigheidstrainingen beschikbaar voor deze
                        competentiecategorie.
                      </div>
                    ) : (
                            ...
                            {filteredTrainings.map((item) => {
                              ...
                              return (
                                ...
                                >
                                  ...
                                  <td className="px-4 py-3 text-sm text-slate-600">
                                    {item.training.est_minutes || "–"}
                                  </td>
                                  <td className="px-4 py-3">
                                    {isTeacherSet ? (
                                      <span
                                        className={`inline-flex items-center rounded-full px-2.5 py-1 text-xs font-semibold ${STATUS_COLORS[item.status]}`}
                                      >

...
export function getFileHint(
  ...
  try {
    ...
    if (
      ...
      // Office Online word path pattern
      return "doc";
    }
    ...
    if (
      ...
      // Office Online PowerPoint path pattern
      return "ppt";
    }
 ...
 */
export function isLikelyToRedirectToLogin(
  url: string | null | undefined,
): boolean {
  if (!url) return false;
  ...
  try {
    ...
    if (
      isHostnameOrSubdomain(hostname, "sharepoint.com") ||
      ...
      return redirectIndicators.some((indicator) =>
...
export function isDirectPdfUrl(url: string | null | undefined): boolean {
  ...
  try {
    ...
    if (
      ...
      return (
        urlLower.includes("download=1") || urlLower.includes("?download=1")
      );
    }
 ...
 */
export function shouldAttemptInlineEmbed(url: string | null | undefined): {
  ok: boolean;
  reason?: string;
} {
  if (!url) {

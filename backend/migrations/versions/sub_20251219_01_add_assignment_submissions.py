...
def upgrade():
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ["project_team_id"], ["project_teams.id"], ondelete="RESTRICT"
        ),
        sa.ForeignKeyConstraint(
            ["submitted_by_user_id"], ["users.id"], ondelete="SET NULL"
        ),
        sa.ForeignKeyConstraint(
            ["last_checked_by_user_id"], ["users.id"], ondelete="SET NULL"
        ),
        sa.PrimaryKeyConstraint("id"),
        sa.CheckConstraint(
            "doc_type IN ('report', 'slides', 'attachment')", name="check_doc_type"
        ),
        sa.CheckConstraint(
            "status IN ('missing', 'submitted', 'ok', 'access_requested', 'broken')",
            name="check_status",
        ),
        sa.UniqueConstraint(
    ...
    op.create_index(
        "ix_submissions_assessment", "assignment_submissions", ["project_assessment_id"]
    )
    op.create_index(
        "ix_submissions_team", "assignment_submissions", ["project_team_id"]
    )
    op.create_index(
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(["school_id"], ["schools.id"], ondelete="CASCADE"),
        sa.ForeignKeyConstraint(
            ["submission_id"], ["assignment_submissions.id"], ondelete="CASCADE"
        ),
        sa.ForeignKeyConstraint(["actor_user_id"], ["users.id"], ondelete="SET NULL"),
        sa.PrimaryKeyConstraint("id"),
        sa.CheckConstraint(
            "event_type IN ('submitted', 'status_changed', 'cleared', 'opened', 'commented')",
    ...
    op.create_index(
        "ix_submission_events_submission", "submission_events", ["submission_id"]
    )
    op.create_index(
        "ix_submission_events_created",
        "submission_events",
        ["created_at"],

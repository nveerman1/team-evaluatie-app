...
import pytest
from app.api.v1.routers.project_assessments import _score_to_grade
...
class TestCurvedGradeMapping:
    ...
    def test_score_clamped_below_min(self):
        ...
        assert _score_to_grade(0.0, 1, 5) == 1.0
    ...
    def test_score_clamped_above_max(self):
        ...
        assert _score_to_grade(6.0, 1, 5) == 10.0
...
class TestOverviewScoreToGrade:
    """Tests for the _score_to_grade function in overview.py"""

    def test_overview_uses_same_curve(self):
        """Verify that overview.py uses the same curved mapping"""
        from app.api.v1.routers.overview import _score_to_grade as overview_score_to_grade

        # Verify the three key test cases match
        assert overview_score_to_grade(1.0, 1, 5) == 1.0
        assert overview_score_to_grade(3.0, 1, 5) == 6.0
        assert overview_score_to_grade(5.0, 1, 5) == 10.0

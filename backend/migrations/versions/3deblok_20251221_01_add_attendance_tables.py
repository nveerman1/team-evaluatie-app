...
def upgrade() -> None:
    ...
    op.create_index('ix_rfid_cards_uid_active', 'rfid_cards', ['uid'], postgresql_where=sa.text('is_active = true'))
    ...
    op.create_index('ix_attendance_events_project_id', 'attendance_events', ['project_id'])
    op.create_index('ix_attendance_events_check_in', 'attendance_events', ['check_in'])
    op.create_index('ix_attendance_events_open_sessions', 'attendance_events', ['user_id', 'check_in'], 
                    postgresql_where=sa.text('check_out IS NULL'))
    op.create_index('ix_attendance_events_external_pending', 'attendance_events', ['user_id', 'approval_status'],
    ...
    op.create_table(
        ...
        sa.Column('id', sa.Integer(), nullable=False),
        sa.Column('user_id', sa.Integer(), nullable=False),
        sa.Column('total_school_seconds', sa.Integer(), nullable=False, server_default='0'),
        sa.Column('total_external_approved_seconds', sa.Integer(), nullable=False, server_default='0'),
        sa.Column('lesson_blocks', sa.Numeric(precision=10, scale=1), nullable=False, server_default='0'),
        sa.Column('last_recomputed_at', sa.DateTime(timezone=True), nullable=False, server_default=sa.text('CURRENT_TIMESTAMP')),
        sa.ForeignKeyConstraint(['user_id'], ['users.id'], ondelete='CASCADE'),
        sa.PrimaryKeyConstraint('id'),
        sa.UniqueConstraint('user_id', name='uq_attendance_aggregates_user_id')
    )
    op.create_index('ix_attendance_aggregates_user_id', 'attendance_aggregates', ['user_id'])

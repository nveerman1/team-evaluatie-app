...
def normalize_and_validate_return_to(raw: Optional[str]) -> Optional[str]:
    ...
    if not normalized.startswith("/"):
        logger.warning(
            f"returnTo does not start with / after normalization: {normalized} (original: {raw})"
        )
        return None

    if normalized.startswith("//"):
        logger.warning(
            f"returnTo starts with // after normalization: {normalized} (original: {raw})"
        )
        return None
    ...
    if "://" in normalized:
        logger.warning(
            f"returnTo contains :// after normalization: {normalized} (original: {raw})"
        )
        return None
    ...
    if ":" in normalized[:20]:  # Check first 20 chars for protocols
        ...
        if protocol_part in dangerous_protocols:
            logger.warning(
                f"returnTo contains dangerous protocol after normalization: {normalized} (original: {raw})"
            )
            return None
